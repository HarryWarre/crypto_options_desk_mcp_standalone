# UI-005 — Comprehensive backend terminal system monitoring
...
- [x] `<details class="technical-log">` is open by default and labeled as the backend monitoring system.
- [x] `appendTerminal` prepends timestamps (`[HH:MM:SS]`), supports status styling (`success`, `error`, `info`), and auto-scrolls to the latest entry.
- [x] Asset discovery flow logs initiation, discovered instruments, and completion status.
- [x] Scan stream (`POST /api/v1/opportunities/scan/stream`) emits structured step logs through `_execute_scan` covering:
...
- [x] Live desk WebSocket stream (`WS /api/v1/opportunities/stream`) logs connection states, cycle iterations, snapshot emissions, and interval countdowns to the terminal.
- [x] Historical backtest flow logs start parameters, data replay progress, and final summary to the terminal.
- [x] Position monitoring flow logs WebSocket connection events, reconciliation, and snapshot decisions (CLOSE, HOLD, REVIEW) to the terminal.
- [x] "Xóa log" button clears the terminal without breaking subsequent logging.
- [x] Python unit/UI tests pass.
- [x] Playwright E2E suite passes and verifies terminal step logging and clear behavior.
